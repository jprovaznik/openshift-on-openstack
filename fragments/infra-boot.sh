...
if [ -e /run/ostree-booted ]; then
    ...
    docker run -d -p 53:53/udp -v /var/log:/var/log -v /etc/hosts:/etc/hosts -v /etc/dnsmasq.conf:/etc/dnsmasq.conf -v /etc/resolv.conf.local:/etc/resolv.conf jprovaznik/ooshift-dns || notify_failure "failed to run dns docker image"
    docker pull jprovaznik/ooshift-heat-agent
    docker run -d --privileged --ipc=host --net=host --pid=host -e HOST=/host -e NAME=rhel-tools -v /run:/run -v /var/log:/var/log -v /etc/localtime:/etc/localtime -v ~/.ssh:/root/.ssh -v /:/host -v /etc/ansible:/etc/ansible -v /var/lib/heat-cfntools:/var/lib/heat-cfntools jprovaznik/ooshift-heat-agent || notify_failure "failed to run heat-agent docker image"
else
    systemctl is-enabled os-collect-config || notify_failure "os-collect-config service is not installed or enabled"
